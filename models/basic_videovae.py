...
# this file only provides the 2 modules used in VQVideoVAE
__all__ = ['CNNVideoEncoder', 'CNNVideoDecoder', ]
...
class Downsample2x(nn.Module):
    ...
    def forward(self, x):
        return self.conv(F.pad(x, pad=(0, 1, 0, 1, 0, 1), mode='constant', value=0))
...
class AttnBlock(nn.Module):
    def __init__(self, in_channels):
        ...
        self.qkv = torch.nn.Conv3d(in_channels, 3*in_channels, kernel_size=1, stride=1, padding=0)
        self.w_ratio = int(in_channels) ** (-0.5)
        self.proj_out = torch.nn.Conv3d(in_channels, in_channels, kernel_size=1, stride=1, padding=0)
    
    def forward(self, x):
        qkv = self.qkv(self.norm(x))
        B, _, T, H, W = qkv.shape  # should be B,3C,T,H,W
        C = self.C
        q, k, v = qkv.reshape(B, 3, C, T, H, W).unbind(1)
        
        # compute attention
        q = q.view(B, C, T * H * W).contiguous()
        q = q.permute(0, 2, 1).contiguous()     # B,THW,C
        k = k.view(B, C, T * H * W).contiguous()    # B,C,THW
        w = torch.bmm(q, k).mul_(self.w_ratio)  # B,THW,THW    w[B,i,j]=sum_c q[B,i,C]k[B,C,j]
        w = F.softmax(w, dim=2)
        
        # attend to values
        v = v.view(B, C, T * H * W).contiguous()
        w = w.permute(0, 2, 1).contiguous()  # B,THW,THW (first THW of k, second of q)
        h = torch.bmm(v, w)  # B, C,THW (THW of q) h[B,C,j] = sum_i v[B,C,i] w[B,i,j]
        h = h.view(B, C, T, H, W).contiguous()
...
class CNNVideoEncoder(nn.Module):
    def __init__(
        ...
        # downsampling
        self.conv_in = torch.nn.Conv3d(img_channels, self.ch, kernel_size=3, stride=1, padding=1)
        ...
        self.conv_out = torch.nn.Conv3d(block_in, output_channels, kernel_size=3, stride=1, padding=1)
...
class CNNVideoDecoder(nn.Module):
    def __init__(
        ...
        # z to block_in
        self.conv_in = torch.nn.Conv3d(input_channels, block_in, kernel_size=3, stride=1, padding=1)
        ...
        # end
        self.norm_out = Normalize(block_in)
        self.conv_out = torch.nn.Conv3d(block_in, img_channels, kernel_size=3, stride=1, padding=1)
...
if __name__ == '__main__':
    import cv2 as cv
    video_path = r"D:\VideoVAR\OpenVid_part108\video\RD3Ddydhpw0_2_0to124.mp4"
