...
class VideoVAETrainer(object):
    def __init__(
        ...
        if self.using_ema:
            self.vae_ema: VQVideoVAE = deepcopy(vae_wo_ddp).eval()
        else:
            self.vae_ema: VQVideoVAE = None
    ...
    def train_step(
        ...
        with maybe_record_function('VAE_rec'):
            with self.vae_opt.amp_ctx:
                ...
                rec_B3THW, Lq, Le, usage = self.vae(inp, ret_usages=loggable)
                B = rec_B3THW.shape[0]
                inp_rec_no_grad = torch.cat((inp, rec_B3THW.data), dim=0)

            Lrec = F.l1_loss(rec_B3THW, inp)
            Lrec_for_log = Lrec.data.clone()
            Lrec *= self.wei_l1
            if self.wei_l2 > 0:
                Lrec += F.mse_loss(rec_B3THW, inp).mul_(self.wei_l2)
            
            using_lpips = inp.shape[-2] >= self.lp_reso and self.wei_lpips > 0
            if using_lpips:
                self.lpips_loss.forward
                Lpip = self.lpips_loss(inp, rec_B3THW)
        ...
        if warmup_disc_schedule > 0:
            with maybe_record_function('VAE_disc'):
                ...
                with self.disc_opt.amp_ctx:
                    ...
                    Lg = -self.disc_wo_ddp(self.daug.aug(rec_B3THW, fade_blur_schedule), grad_ckpt=False).mean()  # todo: aug or not?
    ...
    def __repr__(self):
        return (
            ...
            f'[{type(self).__name__}.structure]: {super(VideoVAETrainer, self).__repr__().replace(VideoVAETrainer.__name__, "")}'
        )
        ...
        for k in ('vae_wo_ddp', 'vae_ema', 'disc_wo_ddp', 'vae_opt', 'disc_opt'):
            ...
            if m is not None:
                ...
                if ret is not None:
                    ...
                    print(f'[VideoVAETr.load_state_dict] {k} missing:  {missing}')
                    print(f'[VideoVAETr.load_state_dict] {k} unexpected:  {unexpected}')
        config: dict = state.pop('config', None)
        if config is not None:
            for k, v in self.get_config().items():
                if config.get(k, None) != v:
                    err = f'[VideoVAETr.load_state_dict] config mismatch:  this.{k}={v} (ckpt.{k}={config.get(k, None)})'
                    if strict:
